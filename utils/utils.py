...
def one_hot(label, num_classes):
    ...
    heat_map = np.zeros(shape=label.shape[0:2] + (num_classes,))
    # colors = np.unique(label.reshape(-1),axis=0) # 255와 0으로 예상

    for i in range(1,num_classes):
        heat_map[:, :, i] = np.equal(label, i).astype('float32') # 이렇게 하면 정상적인 mask가 생성될 것이다.
    return heat_map

    # one-hot encoding 거꾸로 되는거 같은데...?
    # if np.ndim(label) == 3:
    #     label = np.squeeze(label, axis=-1)
    # assert np.ndim(label) == 2
    #
